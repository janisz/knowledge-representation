...
public class Hoent {
    ...
    public boolean hCheckCompatibility(String evaluationToTest, byte time) {
        ...
        for (byte fluentIndex = 0; fluentIndex < fluentsCount; fluentIndex++) {
            ...
            if (charAtIndexInEvalToTest != '?') {
                ...
                else if (charAtIndexInEvalToTest == '0' && charAtIndexInActEval != '0' && charAtIndexInActEval == '1') {
                    return false;
                }
            }
        }
    ...
    public String hGetNewEvaluates(String posEvaluate, byte time) {
        ...
        Fluents.duplicatedCode2((byte) fluentsCount, resultSB, evaluationInH, posEvaluate);
        return resultSB.toString();
    }
    ...
    public void hAddNewEvaluates(String newEvaluates, byte time) {
        ...
        duplicatedCode(newEvaluates, this.fluentsCount, evaluationInH, newEvaluationInH);
    ...
    public static void duplicatedCode(String newEvaluates, short fluentsCount, String evaluationInH, StringBuilder newEvaluationInH) {
        for (byte fluentIndex = 0; fluentIndex < fluentsCount; fluentIndex++) {
    ...
    public boolean eCanInsertActionAtTime(byte actionID, byte timeID) {
        ...
        return timeID < tMax
                ...
                || !ArrayListOfByteUtils.contains(this.sysElemE.get(timeID).disallowedActions, actionID)
    ...
    public boolean oAreSysElemOsTheSame(ArrayList<HashMap<Byte, String>> otherSysElemO) {
        for (byte index = 0; index < this.sysElemN.size(); index++) {
            if (this.sysElemO.get(index).keySet().size() != 0 ||
                ...
                //compare actionID
                else if (Objects.equals(this.sysElemO.get(index).keySet().iterator().next(), otherSysElemO.get(index).keySet().iterator().next())) {
