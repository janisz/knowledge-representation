...
\begin{description}[style=nextline]
	...
	\item[$a_i \texttt{ occurs } \texttt{ at } \pi$]
	akcja $a_i$\footnote{akcja $a_i$ złożona jest z agenta ($\Lambda$) i zadania ($\tau$), które wykonuje} jest wykonywana w czasie $t$
	\item[$\texttt{typically } a_i \texttt{ occurs } \texttt{at } \pi$]
	zazwyczaj akcja $a_i$ jest wykonywana w czasie $t$
	...
	jeśli zachodzi warunek $\pi$
	\item[$a_i \texttt{ releases } f \texttt{ if } \pi$]
	akcja $a_i$ powoduje uwolnienie fluentu $f$,
	jeśli zachodzi warunek $\pi$
	\item[$\texttt{typically } a_i \texttt{ releases } f \texttt{ if } \pi$]
	zazwyczaj akcja $a_i$ powoduje uwolnienie fluentu $f$,
	jeśli zachodzi warunek $\pi$
...
występujących w $\alpha$. Wtedy dla zdań języka $\mathcal{AL}$  muszą być spełnione następujące warunki:
   \begin{itemize}
   		\item Dla każdego wyrażenia $ ( \alpha at t)$, wtedy i tylko wtedy, gdy $H(\alpha, t)=1$   	
		...
		\item Dla każdego wyrażenia $ ( typically\; a_i\;causes\;\alpha\;if\;\pi)\in D $ i dla każdego momentu w czasie
		 ...
		 \item Dla każdego wyrażenia $ ( a\; occurs\; at \; t)$, wtedy i tylko wtedy, gdy $(A, t) \in E$
		 \item Dla każdego wyrażenia $ ( typically \; a \; occurs \; at\; t)$, wtedy i tylko wtedy, gdy jeżeli $(A, t) \in N$ to $(A, t) \in E$
