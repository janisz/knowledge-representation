...
Następnie w czasie 3 Adam typowo kupi bilet będąc w kinie, po czym w czasie 4, również typowo, zacznie oglądać film.
	...
	\begin{align*}
	&\texttt{initially } hungry, \neg hasTicket, atHome, \neg atCinema, \neg isMovieAvailable \\
	&hungry \texttt{ triggers } Adam Eat \\
	&Adam Eat \texttt{ causes } \neg hungry \\
	&Adam WashTeeth \texttt{ at } 1 \\
	&\texttt{typically } \neg hasTicket, atHome \texttt{ triggers } Adam BuyTicketByTheInternet \\
	&Adam BuyTicketByTheInternet \texttt{ causes } hasTicket \texttt{ if } atHome \\
	&Adam GoToTheCinema \texttt{ at } 2 \\
	&Adam GoToTheCinema \texttt{ causes } \neg atHome, atCinema \\
	&\texttt{typically } atCinema, \neg hasTicket \texttt{ triggers } Adam BuyTicketAtTheCinema \\
	&Adam BuyTicketAtTheCinema \texttt{ causes } hasTicket \texttt{ if } atCinema, \neg hasTicket \\
	&isMovieAvailable \texttt{ at } 3 \\
	&\neg isMovieAvailable \texttt{ at } 5 \\
	&hasTicket, isMovieAvailable \texttt{ typically causes } Adam WatchMovie
	\end{align*}
\end{example}
...
Oznaczenia fluentów: roomClosed(r) , hostelClosed(h), inRoom(i), hasCard(c)
	...
	&DoorKeeper closeDoor \texttt{ causes } hostelClosed \\
	&\texttt{typically } \neg hasCard \texttt{ triggers } Janek TakeCard \texttt{ if } inRoom \\
	&\texttt{typically } Janek Leave \texttt{ invokes } Janek LockTheRoom \\
	&Janek LockTheRoom \texttt{ causes } roomClosed \\
	&Janek TakeCard \texttt{ causes } hasCard \\
	&Janek Leave \texttt{ causes } \neg inRoom \\
	&Janek Comeback \texttt{ causes } inRoom, \neg roomClosed \texttt{ if } hasCard, roomClosed, \neg inRoom, \neg hostelClosed \\
	&Janek Comeback \texttt{ causes } inRoom \texttt{ if } \neg inHostel, \neg hostelClosed \\
	&\texttt{typically } DoorKepper CloseDoor \texttt{ occurs at } 4 \\
	&Janek Leave \texttt{ occurs at } 1 \\
	&Janek Comeback \texttt{ occurs at } 3
	\end{align*}
